...
namespace ProRob
{
    public class NpoiHelper
    {
        ...
        public static DataTable ImportExcel2007toDt(string strFileName, int sheetIndex)
        {
            DataTable dt = new DataTable();
            IWorkbook hssfworkbook;
            using (FileStream file = new FileStream(strFileName, FileMode.Open, FileAccess.Read))
        ...
        public static void UpdateExcelAsNew(string outputFile, string sourceFile, string sheetname, object[][] updateData, int coluid, int rowid)
        {
            ...
            for (int i = 0; i < updateData.Length; i++)
            {
                for (int j = 0; j < updateData[i].Length; j++)
                {
                    try
                    {
                        ...
                        if (sheet1.GetRow(i + rowid).GetCell(j + coluid) == null)
                        {
                            sheet1.GetRow(i + rowid).CreateCell(j + coluid);
                        }


                        if (updateData[i][j] is int)
                        {
                            int val = (int)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
                        else if (updateData[i][j] is long)
                        {
                            long val = (long)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
                        else if (updateData[i][j] is double)
                        {
                            double val = (double)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
                        else if (updateData[i][j] is bool)
                        {
                            bool val = (bool)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
                        else if (updateData[i][j] is DateTime)
                        {
                            DateTime val = (DateTime)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
                        else if (updateData[i][j] is IRichTextString)
                        {
                            IRichTextString val = (IRichTextString)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
                        else
                        {
                            string val = (string)updateData[i][j];
                            sheet1.GetRow(i + rowid).GetCell(j + coluid).SetCellValue(val);
                        }
        ...
        public static void TestDoWork(string outputFile, string sourceFile)
        {

            IWorkbook hssfworkbook_src;
            
            using (FileStream file_src = new FileStream(sourceFile, FileMode.Open, FileAccess.Read))
            {
                hssfworkbook_src = new XSSFWorkbook(file_src);
            }
            /*
            using (FileStream file = new FileStream(sourceFile, FileMode.Open, FileAccess.Read))
            {
                hssfworkbook_src = new HSSFWorkbook(file);
            }
            */
            try
            {
                FileStream writefile = new FileStream(outputFile, FileMode.Create);
                hssfworkbook_src.Write(writefile);
                writefile.Close();
            }
            catch (Exception ex)
            {
        ...
        public static void CopyCell(ICell cell, IWorkbook iworkbook, ISheet sheet, int left, int top)
        {
            if (cell == null)
                return;

            if (sheet.GetRow(top) == null)
                sheet.CreateRow(top);

            if (sheet.GetRow(top).GetCell(left) != null)
                sheet.GetRow(top).RemoveCell(sheet.GetRow(top).GetCell(left));

            ICell newCell = sheet.GetRow(top).CreateCell(left);
            newCell.SetCellType(cell.CellType);

            ICellStyle cellstyle = iworkbook.CreateCellStyle();
            ...
            switch (cell.CellType)
            {
                ...
                case CellType.Numeric:
                    ...
                    {
                        newCell.SetCellValue(DateTime.FromOADate(cell.NumericCellValue));
                    }
                    else
                    {
                        newCell.SetCellValue(Convert.ToDouble(cell.NumericCellValue));
                    }
                    break;
                case CellType.Boolean:
                    newCell.SetCellValue(Convert.ToString(cell.BooleanCellValue));
                    break;
                case CellType.Error:
                    newCell.SetCellValue(ErrorEval.GetText(cell.ErrorCellValue));
                    break;
                case CellType.Formula:
                    switch (cell.CachedFormulaResultType)
                    {
                        case CellType.String:
                            ...
                            if (strFORMULA != null && strFORMULA.Length > 0)
                            {
                                newCell.SetCellValue(strFORMULA.ToString());
                            }
                            else
                            {
                            }
                            break;
                        case CellType.Numeric:
                            newCell.SetCellValue(Convert.ToString(cell.NumericCellValue));
                            break;
                        case CellType.Boolean:
                            newCell.SetCellValue(Convert.ToString(cell.BooleanCellValue));
                            break;
        ...
        public static void DoWork(string outputFile, string sourceFile, string ModeFile, int[][] updateData, bool all)
        {
            IWorkbook hssfworkbook_src;
            using (FileStream file_src = new FileStream(sourceFile, FileMode.Open, FileAccess.Read))
            {
            ...
            ISheet sheet_src = hssfworkbook_src.GetSheetAt(0);

            IWorkbook hssfworkbook_mod;
            using (FileStream file_mod = new FileStream(ModeFile, FileMode.Open, FileAccess.Read))
            {
                hssfworkbook_mod = new XSSFWorkbook(file_mod);
            }
            ISheet sheet_mod = hssfworkbook_mod.GetSheetAt(0);

            IWorkbook hssfworkbook_out;
            using (FileStream file_mod = new FileStream(ModeFile, FileMode.Open, FileAccess.Read))
            {
                hssfworkbook_out = new XSSFWorkbook(file_mod);
            }

            int num = hssfworkbook_out.NumberOfSheets;
            for (int i = 0; i < num; i++)
                hssfworkbook_out.RemoveSheetAt(0);
            ISheet sheet_out = hssfworkbook_out.CreateSheet();
            
            short col_src = sheet_src.GetRow(0).LastCellNum;
            short col_mod = sheet_mod.GetRow(0).LastCellNum;
            ...
            for (int i = 0; i < updateData.Length; i++)
            {
                ...
                if (!all)
                {
                    for (int j = 0; j < col_src; j++)
                    {
                        ICell cell_src = sheet_src.GetRow(updateData[i][0]).GetCell(j);
                        CopyCell(cell_src, hssfworkbook_out, sheet_out, j, i);
                    }
                }

                for (int j = 0; j < col_mod; j++)
                {
                    if (all)
                    {
                        ICell cell_mod = sheet_mod.GetRow(updateData[i][1]).GetCell(j);
                        CopyCell(cell_mod, hssfworkbook_src, sheet_src, col_src + j, updateData[i][0]);
                    }
                    else
                    {
                        ICell cell_mod = sheet_mod.GetRow(updateData[i][1]).GetCell(j);
                        CopyCell(cell_mod, hssfworkbook_out, sheet_out, col_src + j, i);
                    }
                }
            ...
            try
            {
                ...
                if(all)
                    ...
                    hssfworkbook_out.Write(writefile);
        ...
        public static bool DoWork(string outputFile, string ModeFile, int[][] updateData)
        {
            IWorkbook hssfworkbook_mod;
            using (FileStream file_mod = new FileStream(ModeFile, FileMode.Open, FileAccess.Read))
            {
            ...
            for (int i = 0; i < updateData.Length; i++)
            {
                ...
                for (int j = 0; j < col_mod; j++)
                {
                    ICell cell_mod = sheet_mod.GetRow(updateData[i][0]).GetCell(j);
                    CopyCell(cell_mod, hssfworkbook_out, sheet_out, j, i);
                }
            }
